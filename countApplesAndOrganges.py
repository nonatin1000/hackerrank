...
The red region denotes the house, where s is the start point, a and t is the endpoint.
...
distance from its tree of origin along the x-axis. *A negative value of d means the fruit fell d
...
For example, Sam's house is between s=7 and t=10. The apple tree is located at b=12 and the orange at .
...
the region between 7 and 11, so we print 1 as our first line of output.
...
def countApplesAndOranges(s, t, a, b, apples, oranges):
    count_apples = sum(a + apple >= s and a + apple <= t for apple in apples)
    count_oranges = sum(b + orange >= s and b + orange <= t for orange in oranges)
    print(count_apples)
    print(count_oranges)
